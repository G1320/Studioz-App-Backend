...
const getUserWishlistById = handleRequest(async (req) => {
    ...
    const wishlistIndex = wishlists.findIndex((wishlist) => wishlist._id === req.params.wishlistId);
    if (wishlistIndex === -1)
        throw new ExpressError('Wishlist not found', 404);
    ...
    const nextWishlist = wishlistIndex < user.wishlists.length - 1 ? user.wishlists[wishlistIndex + 1] : null;

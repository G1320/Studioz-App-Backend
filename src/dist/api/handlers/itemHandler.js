...
const createItem = handleRequest(async (req) => {
    ...
    if (!studioId)
        throw new ExpressError('studio ID not provided', 400);
    const itemId = item._id;
    if (!itemId)
        throw new ExpressError('item ID not provided', 400);
    ...
    await item.save();
    if (studio.items.length > 31)
        throw new ExpressError('Oops, Studio is full!', 400);
    if (studio.items.some((studioItem) => studioItem.itemId.toString() === item._id.toString())) {
...
const removeItemFromStudio = handleRequest(async (req) => {
    ...
    const itemIndex = studio.items.findIndex((studioItem) => studioItem.itemId.toString() === itemIdToRemove.toString());
    if (itemIndex === -1)
        throw new ExpressError('item not found in the studio', 404);
    // Remove the item at the found index
    studio.items.splice(itemIndex, 1);
    // Re-map the idx values for the remaining items
    studio.items.forEach((studioItem, index) => (studioItem.idx = index));
    // Save the studio
    await studio.save();
...
const addItemToWishlist = handleRequest(async (req) => {
    ...
    if (!wishlistId)
        throw new ExpressError('wishlist ID not provided', 400);
    const itemId = req.params.itemId;
    if (!itemId)
        throw new ExpressError('item ID not provided', 400);
    const wishlist = await WishlistModel.findById(wishlistId);
    if (!wishlist)
        throw new ExpressError('wishlist not found', 404);
    if (!wishlist.items)
        wishlist.items = [];
    const item = await ItemModel.findById(itemId);
    if (!item)
    ...
    item.updatedAt = new Date();
    await item.save();
    if (wishlist.items.length > 31)
        throw new ExpressError('Oops, Wishlist is full!', 400);
    if (wishlist.items.some((wishlistItem) => wishlistItem.itemId.toString() === item._id.toString())) {
...
const removeItemFromWishlist = handleRequest(async (req) => {
    ...
    if (itemIndex === -1)
        throw new ExpressError('Item not found in the wishlist', 404);
